...
export default function MainReserve() {
  ...
  return (
    <main ref={containerRef} className="w-full max-w-6xl mx-auto px-6 py-12 space-y-12 relative min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        {/* Info Left */}
        <div className="animate-panel-left lg:col-span-4 bg-white border border-neutral-200 rounded-3xl p-8 space-y-4">
        ...
        {/* Input Form Right */}
        <div className="animate-panel-right lg:col-span-8 bg-white rounded-3xl p-6 sm:p-8 border border-neutral-200 space-y-6">
          <div className="flex justify-center">
            <div className="bg-neutral-100 p-1.5 rounded-2xl flex relative w-full max-w-sm border">
          ...
          {submitState.status === 'success' && (
            <div className="p-4 bg-green-50 border text-green-800 text-sm font-bold rounded-2xl">
              Booking confirmed! Assigned Allocation Number: #{submitState.allocated}
            </div>
          )}

          {submitState.status === 'error' && (
            <div className="p-4 bg-red-50 border text-red-700 text-sm font-bold rounded-2xl">
              {submitState.message}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full p-3.5 rounded-xl border text-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-neutral-700">Mobile Number</label>
              <input type="tel" value={mobile} onChange={(e) => setMobile(e.target.value)} required className="w-full p-3.5 rounded-xl border text-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-neutral-700">Select Date</label>
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} required className="w-full p-3.5 rounded-xl border text-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-neutral-700">Options</label>
              <select value={dynamicSelection} onChange={(e) => setDynamicSelection(e.target.value)} className="w-full p-3.5 rounded-xl border text-sm bg-white">
                {bookingType === 'seat' 
                    ...
                    <button
                      ...
                      className={`py-3 px-4 border rounded-xl text-xs font-bold transition-all block w-full text-center ${
                        isSelected
                          ...
                          : 'border-neutral-200 text-neutral-600 bg-neutral-50/50 hover:border-neutral-400'
                      }`}
                    >
                      {time}
                    </button>
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <textarea rows={3} value={specialRequests} onChange={(e) => setSpecialRequests(e.target.value)} className="w-full p-3.5 rounded-xl border text-sm resize-none" />
            </div>

            <div className="md:col-span-2 pt-2">
              <button type="submit" className="w-full bg-orange-500 text-white py-4 rounded-xl font-bold hover:bg-orange-600 transition-colors text-sm">
                Confirm Your Space
              </button>
            </div>
          </form>
